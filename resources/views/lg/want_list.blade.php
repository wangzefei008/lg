@include('components.head')<!-- end #header -->
	<div id="container">
...
-->
		...
		<div class="content">
					...
					<li class="odd clearfix">
						<div class="hot_pos_l">
							<div class="mb10">
								...
								&nbsp;
								<span class="c9">{{ $value->residence }}</span>
							</div>
							<span><em class="c7">姓名： </em>{{ $value->fullname}}</span>
							<span><em class="c7">性别： </em>{{ $value->sex_cn }}</span>
							<span><em class="c7">工作经验：</em>{{ $value->experience_cn }}</span>
							<span><em class="c7">出生年份： </em>{{ $value->birthdate }}</span>
							<br/>
							<span><em class="c7">期望薪资：</em>{{ $value->wage_cn }}</span>
							<br/>
							<span>@if(floor((time()-$value->addtime)/3600/24)==0)
									{{'今天发布'}}
								@else {{floor((time()-$value->addtime)/3600/24)}}天前发布</span>
							@endif
						</div>
						<div class="hot_pos_r">
							<div class="apply">
							...
							<span><em class="c7">所学专业： </em>{{ $value->major_cn }}</span>
							<span><em class="c7">身高：</em>{{ $value->height }}</span>
							<br />
							<span><em class="c7">应聘职位： </em>{{ $value->trade_cn }}</span>
							<span><em class="c7">婚姻状况： </em>{{ $value->marriage_cn}}</span>
			...
			<div class="Pagination">{!! $job->links() !!}</div>
		</div>
