    ...
    <div id="container">
            ...
            <div class="content_l">
                    ...
                    <dd class="job_request">
						<span class="red">居住地：{{$residence}}</span>
						<span>工作经验：{{$experience_cn}}</span>
						<span>学历：{{$education_cn}}</span><br/>
						<span> 工作性质：{{$nature_cn}}</span>
						<span>婚姻状况：{{$marriage_cn}}</span>
						<span>籍贯：{{$householdaddress}} </span><br/>
						<span>期望薪资：{{$wage_cn}}</span>
						<span>所学专业：{{$major_cn}}</span><br/>
						<span>身高：{{$height}}</span>
						<span> 期望职位：{{$intention_jobs}}</span>
						<br/>
						·&nbsp;手机 : {{$telephone}}<br/>
						·&nbsp;邮箱 : {{$email}}<br/>
						·&nbsp;出生年份 : {{$birthdate}}<br/>
						·&nbsp;特长: {{$tag_cn}}
						<div>发布时间：@if(floor((time()-$addtime)/3600/24)==0)
								{{'今天发布'}}
							@else {{floor((time()-$addtime)/3600/24)}}天前发布</span>
							@endif</div>
                    </dd>
                    <dd class="job_bt">
                        <h3 class="description">自我描述：</h3>
							<p style="margin-left: 50px;">{{$specialty}}</p>
                    </dd>
