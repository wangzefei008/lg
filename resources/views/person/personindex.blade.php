    ...
    <div id="container">
        ...
        <div class="content">
            <dl class="company_center_content">
                ...
                <h1>
                    <em></em>
                    基本资料 <span><i style="color:#fff;font-style:normal" id="positionNumber">{{$email}}</i></span>                        </h1>
                </dt>
                <dd>
                    <form id="searchForm">
                        ...
                        <ul class="reset my_jobs">
                            <li data-id="149594">
                                <h3>
                                    <a target="_blank" title="姓名" href="">{{$realname}}</a>
                                    <span>{{$sex_cn}}</span><span>{{$birthday}}</span><span>{{$height}}cm</span><span>{{$householdaddress}}</span>
                                </h3>
                                <span class="receivedResumeNo"><a href="{{URL::to('person_update')}}">修改</a></span>
                                <div>{{$residence}}/ {{$education_cn}} / {{$major_cn}} / {{$experience_cn}}</div>
                                <div>{{$phone}} /{{$email}}/ {{$marriage_cn}} </div>
                            </li>
                        </ul>
                    </form>
                </dd>
